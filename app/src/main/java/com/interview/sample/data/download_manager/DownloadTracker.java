...
public class DownloadTracker {
    ...
    public void toggleDownload(FragmentManager fragmentManager, MediaItem mediaItem, RenderersFactory renderersFactory) {
        ...
        if (download != null && download.state != Download.STATE_FAILED) {
            DownloadService.sendRemoveDownload(context, OfflineVideoDownloadService.class, download.request.id, /* foreground= */ false);
        } else {
            if (startDownloadDialogHelper != null) {
                startDownloadDialogHelper.release();
            }
            startDownloadDialogHelper = new StartDownloadDialogHelper(fragmentManager, DownloadHelper.forMediaItem(context, mediaItem, renderersFactory, dataSourceFactory), mediaItem);
        }
    }
    ...
    private final class StartDownloadDialogHelper implements DownloadHelper.Callback, TrackSelectionDialog.TrackSelectionListener, DialogInterface.OnDismissListener {
        ...
        public void onPrepared(DownloadHelper helper) {
            ...
            widevineOfflineLicenseFetchTask = new WidevineOfflineLicenseFetchTask(format, mediaItem.localConfiguration.drmConfiguration, dataSourceFactory,
                    /* dialogHelper= */ this, helper);
        ...
        private void onDownloadPrepared(DownloadHelper helper) {
            ...
            trackSelectionDialog = TrackSelectionDialog.createForTracksAndParameters(
                    /* titleId= */ R.string.exo_download_description, tracks, DownloadHelper.getDefaultTrackSelectorParameters(context),
        ...
        private void startDownload(DownloadRequest downloadRequest) {
            DownloadService.sendAddDownload(context, OfflineVideoDownloadService.class, downloadRequest, /* foreground= */ false);
        }

        private DownloadRequest buildDownloadRequest() {
            long timestamp = System.currentTimeMillis();
            String title = mediaItem.mediaMetadata.title != null ? mediaItem.mediaMetadata.title.toString() : Long.toString(timestamp);
            return downloadHelper.getDownloadRequest(Util.getUtf8Bytes(title)).copyWithKeySetId(keySetId);
        }
    }
    ...
    private static final class WidevineOfflineLicenseFetchTask extends AsyncTask<Void, Void, Void> {
        ...
        public WidevineOfflineLicenseFetchTask(Format format, MediaItem.DrmConfiguration drmConfiguration, DataSource.Factory dataSourceFactory, StartDownloadDialogHelper dialogHelper, DownloadHelper downloadHelper) {
            this.format = format;
        ...
        protected Void doInBackground(Void... voids) {
            OfflineLicenseHelper offlineLicenseHelper = OfflineLicenseHelper.newWidevineInstance(drmConfiguration.licenseUri.toString(), drmConfiguration.forceDefaultLicenseUri, dataSourceFactory, drmConfiguration.licenseRequestHeaders, new DrmSessionEventListener.EventDispatcher());
            try {
